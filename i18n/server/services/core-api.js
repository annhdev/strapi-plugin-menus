...
const createLocalizationRoute = (contentType) => {
  ...
  if (!plugin) {
    ...
    return {
      method: 'POST',
      path: routePath,
      handler: `plugin::${plugin}.${modelName}.createLocalization`,
      config: {
        policies: [],
      },
    };
...
const addCreateLocalizationAction = (contentType) => {
  const {modelName, apiName, plugin, uid} = contentType;
  ...
  if (plugin) {
    /**
     * Add controller to the plugin's route
     */
    strapi.container.get('controllers').extend(`plugin::${plugin}.${modelName}`, (controller) => {
      return Object.assign(controller, {
        createLocalization: createLocalizationHandler(contentType),
      });
    });
