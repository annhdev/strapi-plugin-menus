...
module.exports = createCoreService(UID_MENU_ITEM, ({strapi}) => ({
  async bulkCreateOrUpdate(items, menuId) {
    ...
    const updatedItems = [];
    try {
      for (let i in itemsToUpdate) {
        let item = itemsToUpdate[i];
        const sanitizedItem = sanitizeEntity(item);

        let _updatedItem = await strapi.query(UID_MENU_ITEM).update({where: {id: item.id}, data: sanitizedItem}, {transacting});

        updatedItems.push(_updatedItem);
      }
    } catch (err) {
